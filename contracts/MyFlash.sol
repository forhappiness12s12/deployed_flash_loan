...
contract myFlash is Initializable {
    ...
    function initialize(address _owner, address _receiver) public initializer {
        owner = _owner;
        receiver = _receiver;
    ...
    function estimateProfit(
        ...
        (uint256 amount0, uint256[2] memory dexNames) = getEstimate(
            token,
            amount,
            pairToken
        );
    ...
    function transferReceiver(address newReceiver) external onlyOwner {
        require(newReceiver != address(0), "New owner is the zero address");
